...
public class TEA_P2_1211_Transition : SplatoonScript
{
    ...
    private void SetNextFlareElement(string elementName, Vector2 current, bool enabled = true)
    {
        if(Controller.TryGetElementByName(elementName, out var element))
        {
            ...
            {
                if(!(Vector2.Distance(current, _flarePositions[i]) < 5f))
                    continue;
    ...
    public override void OnSettingsDraw()
    {
        ...
        ImGuiEx.HelpMarker(
            Loc(
                en: "The message that will be displayed on your bait.",
                jp: "あなたの番号の立ち位置に表示されるメッセージ。"));

        var showString = C.BaitMessageIS.Get();
        C.BaitMessageIS.ImGuiEdit(
            ref showString,
            "The message that will be displayed on your bait.");
        ...
        var n_s_dir =
            C.N_S_PriorizeDirection == HawkBlastDirection.North ? 0 : 1;

        ImGui.RadioButton("North##north_dir", ref n_s_dir, 0);
        ImGui.SameLine();
        ImGui.RadioButton("South##south_dir", ref n_s_dir, 1);

        C.N_S_PriorizeDirection =
            n_s_dir == 0
                ? HawkBlastDirection.North
                : HawkBlastDirection.South;

        ImGui.Text("Northeast-Southwest");
        ImGui.SameLine();

        var ne_sw_dir =
            C.NE_SW_PriorizeDirection == HawkBlastDirection.Northeast ? 0 : 1;

        ImGui.RadioButton("Northeast##ne_dir", ref ne_sw_dir, 0);
        ImGui.SameLine();
        ImGui.RadioButton("Southwest##sw_dir", ref ne_sw_dir, 1);

        C.NE_SW_PriorizeDirection =
            ne_sw_dir == 0
                ? HawkBlastDirection.Northeast
                : HawkBlastDirection.Southwest;

        ImGui.Text("West-East");
        ImGui.SameLine();

        var w_e_dir =
            C.E_W_PriorizeDirection == HawkBlastDirection.West ? 0 : 1;

        ImGui.RadioButton("West##west_dir", ref w_e_dir, 0);
        ImGui.SameLine();
        ImGui.RadioButton("East##east_dir", ref w_e_dir, 1);

        C.E_W_PriorizeDirection =
            w_e_dir == 0
                ? HawkBlastDirection.West
                : HawkBlastDirection.East;
        ...
        ImGuiEx.HelpMarker(
            Loc(
                en: "Display flares on the ground to indicate the next safe spot.",
                jp: "次の安全地帯を示すために地面にフレアを表示します。"));
    ...
    private class Config : IEzConfig
    {
        ...
        public HawkBlastDirection NE_SW_PriorizeDirection =
            HawkBlastDirection.Northeast;

        public HawkBlastDirection E_W_PriorizeDirection =
            HawkBlastDirection.East;

        public HawkBlastDirection SE_NW_PriorizeDirection =
            HawkBlastDirection.Southeast;
    }
}
